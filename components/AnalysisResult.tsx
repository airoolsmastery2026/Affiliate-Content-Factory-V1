  ...
  return (
    <div className="bg-gray-800 rounded-xl border border-gray-700 overflow-hidden shadow-lg animate-fade-in-up">
      <div className="bg-gradient-to-r from-purple-900/50 to-blue-900/50 p-4 border-b border-gray-700 flex justify-between items-center">
        ...
        <span className="text-xs bg-purple-900 text-purple-200 px-2 py-1 rounded border border-purple-700">
      ...
      <div className="p-6 grid gap-8">
        ...
        <div className="grid md:grid-cols-2 gap-6">
          <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-700">
            <h3 className="text-purple-400 text-sm font-bold uppercase tracking-wider mb-2">Summary</h3>
            <p className="text-gray-300 text-sm leading-relaxed">{data.summary}</p>
          </div>
          <div className="bg-gray-900/50 p-4 rounded-lg border border-gray-700">
             <h3 className="text-blue-400 text-sm font-bold uppercase tracking-wider mb-2">Structure Analysis</h3>
             <ul className="space-y-2 text-sm text-gray-400">
                <li><span className="text-gray-200 font-semibold">Format:</span> {data.structure.format_type}</li>
                <li><span className="text-gray-200 font-semibold">Hook:</span> {data.structure.hook_analysis}</li>
                <li><span className="text-gray-200 font-semibold">CTA:</span> {data.structure.cta_analysis}</li>
             </ul>
          </div>
        </div>

        {/* Psychological Insights (The Core) */}
        <div className="bg-gray-900 p-5 rounded-xl border border-gray-600 relative overflow-hidden">
          ...
          <div className="grid md:grid-cols-3 gap-4 relative z-10">
            ...
            <div className="bg-red-900/20 p-3 rounded-lg border border-red-900/50">
               ...
               <ul className="space-y-1">
                  {data.insights.pains.map((p, i) => (
                    <li key={i} className="text-xs text-red-200">• {p}</li>
                  ))}
               </ul>
            </div>
            ...
            <div className="bg-green-900/20 p-3 rounded-lg border border-green-900/50">
               <h4 className="text-green-400 text-xs font-bold uppercase mb-2 flex items-center">
            ...
            <div className="bg-yellow-900/20 p-3 rounded-lg border border-yellow-900/50">
               <h4 className="text-yellow-400 text-xs font-bold uppercase mb-2 flex items-center">
                  <span className="w-2 h-2 bg-yellow-500 rounded-full mr-2"></span> False Beliefs
               </h4>
               <ul className="space-y-1">
                  {data.insights.false_beliefs.map((f, i) => (
                    <li key={i} className="text-xs text-yellow-200">• {f}</li>
                  ))}
               </ul>
            </div>
          </div>
        ...
        {/* Ideas */}
        <div>
           <h3 className="text-yellow-400 text-sm font-semibold uppercase tracking-wider mb-3">Proposed Content Ideas</h3>
           <div className="grid md:grid-cols-2 gap-3">
              {data.ideas.map((idea, i) => (
                <div key={i} className="bg-gray-700/50 p-3 rounded border border-gray-600 hover:bg-gray-700 transition">
                   <div className="flex justify-between items-start mb-1">
                      <span className="text-white font-bold text-sm">{idea.title}</span>
                      <span className="text-[10px] bg-gray-900 text-gray-400 px-1 rounded uppercase">{idea.video_type}</span>
                   </div>
                   <p className="text-xs text-gray-400">{idea.short_description}</p>
                </div>
              ))}
           </div>
        </div>
